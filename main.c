...
int main(void){
    ...
    GameMode gameMode = {.Diff = NORMAL,.boxspeedMult = 1.0f,.allowCamControl = true,.randomCam = false};
    ...
    while (!WindowShouldClose()){
        ...
        updateCamT(&cam, &camT, mapCenterX, mapCenterZ);
        if (animCam){
            animT += GetFrameTime();
            if (animT < animD){
                float t = animT / animD;
            ...
            } else{animCam = false;cam.position = tarCamPos;}
        }
        ...
        else if (state == STATE_MODE)
        {
            if (IsKeyPressed(KEY_ONE))
                gameMode.Diff = NORMAL;

            if (IsKeyPressed(KEY_TWO))
                gameMode.Diff = MEDIUM;
            ...
            if (gameMode.allowCamControl && !animCam && !won && !fail){
            ...
            if (!block.mov && !won && !fail){
                ...
                if (IsKeyPressed(KEY_UP)){
                    ...
                    moved = true;
                    if(gameMode.randomCam)
                        randomCam(&cam, &camT, mapCenterX, mapCenterZ, maxDim);
            ...
            // updateblox(&block, dt);
            updateblox(&block, dt * gameMode.boxspeedMult);
            ...
            drawbox(&block);
            EndMode3D();
            DrawStatsHUD(stats, gameMode, currlev);
            
            if (fail){
                if (!animCam) {animCam = true;animT = 0.0f;initCamPos = cam.position;tarCamPos = (Vector3){cam.position.x, cam.position.y + 10.0f, cam.position.z + 10.0f};}
